...
doc = """
Sequential/Vertical Game 5 Players
"""


class C(BaseConstants):
    NAME_IN_URL = 'teamwork_V5'
    PLAYERS_PER_GROUP = 5
    NUM_ROUNDS = 9
    endowment = 45
    clear = 14
...
def def_group_var(group: Group): #function to create the random list of target and timer and a random number that selects the round for payment
    group.session.vars['timer_list'] = [random.randint(60, 70) for _ in range(9)]
    levels = [75, 125, 210]
    ...
    def live_method(player, incre):
        ...
        if incre == 10 or incre == 1:
            ...
            if maxi>0 :
                ...
                group.remaining = group.target - group.total_contribution
                remainE = C.endowment - player.contribution
                response = dict(playerc=player.contribution, playert=group.remaining,no=group.cur_editor, remainE=remainE, change=0, groupcontri = group.total_contribution)
                return {0: response}
            else:
            ...
            group.cur_editor = group.cur_editor + 1
            players = player.group.get_players()
            contributions = [p.contribution for p in players]
            group.total_contribution = sum(contributions)
            if group.remaining == 999:
                group.remaining = group.target
            response = dict(no=group.cur_editor, playerc=0, playert=group.remaining, remaine=C.endowment, change=1, groupcontri = group.total_contribution)
            return {0: response}
